...
async function runFullPipeline(url, label) {
  ...
  console.log('Brand Essence:', brandDNA.businessIdentity.essence);
  console.log('Value Proposition:', brandDNA.positioning.statement);
  console.log('Primary CTA:', brandDNA.conversionStrategy.primaryCTA?.text);
  
  // Digital audit object that matches what WebsiteStrategyService expects
  const digitalAudit = {
    score: 50,
  ...
  console.log('Section types:', landingPageSpec.sections?.map(s => s.type));
